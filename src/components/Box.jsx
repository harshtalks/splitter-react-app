...
const BoxComponent = () => {
  ...
  useEffect(() => {
    ...
    if (people) {
      setAmount(bill);
      setTotal(bill * (1 + tip / 100));
    }
  }, [bill, tip, people, amount, total]);
  const changeInBillValue = (e) => {
    setBill(e.target.value);
  };
  const changeInPeople = (e) => {
          ...
          <div className="tip">
            {tip_values.map((el) => (
              <button
                disabled={isOpened}
                className={`${
                  tip === el.tip || (isOpened && el.tip === -1) ? "active" : ""
                } tip_value`}
                key={el.id}
                onClick={() => {
                  if (el.amount === "Custom") {
        ...
        <People>
          ...
          <div className="input">
            ...
            <input
              ...
              placeholder="0"
              value={people}
              onChange={changeInPeople}
            />
          </div>
        </People>
      </BillingComponent>
      <DisplayComponent>
        <div className="money_component">
          <Amount>
            <p>Amount</p>
            <h2>${amount ? (amount / people).toFixed(2) : "0.00"}</h2>
          </Amount>
          <Total>
            <p>Total</p>
            <h2>${total ? (total / people).toFixed(2) : "0.00"}</h2>
          </Total>
        </div>
        ...
        <CustomTip>
          <div className="inputAmount">
            <input
              ...
              onBlur={(e) => setTip(e.target.value)}
              placeholder="Enter Value"
            />

            <button className="tip_value" onClick={() => setIsOpened(false)}>
              Enter
            </button>
          </div>
        </CustomTip>
